...
const ValidateCalculus = yup.object({
  numbercoins: yup
    .string()
    .required()
    .test("bigger-than-0", "Input must be greater than 0", (val) => {
...
const CalculateAPY = () => {
  return (
    <ImageBackground
      source={require("../../assets/backgroundCalculator2.jpg")}
      resizeMode="cover"
      style={styles.image}
    >
      <Formik
            ...
            {apiCalc.map((item, index) => (
              <View key={index}>
                ...
                <Text style={styles.error}>
                  {formikProps.touched[item.value] &&
                    formikProps.errors[item.value]}
                </Text>
              </View>
            ))}
            <TouchableOpacity
